//package Electricity;
//
//import javax.swing.*;
//import java.awt.*;
//
//public class Splash {
//    public static void main(String[] args) {
//        Fframe f1 = new Fframe();
//        f1.setVisible(true);
//        int i ;
//        int speed = 1;
//        for (i=2 ; i<600 ; i+=4 , speed+=2){
//            f1.setLocation(800 -((i+speed)/2),500-(i/2));
//            f1.setSize(i+speed,i);
//            try{
//                Thread.sleep(20);
//            }catch (Exception e){}
//
//        }
//    }
//}
//class Fframe extends JFrame implements Runnable
//{
//    Thread t1 ;
//    Fframe()
//    {
//        super("MAHARASHTRA ELECTRICITY BOARD");
//        setLayout(new FlowLayout());
//        ImageIcon c1 = new ImageIcon(ClassLoader.getSystemResource("icon/desk.jpg"));
//        Image i1 = c1.getImage().getScaledInstance(1000,700,Image.SCALE_DEFAULT);
//        ImageIcon i2 = new ImageIcon(i1);
//        JLabel l1 = new JLabel(i2);
//        add(l1);
//        t1= new Thread(this);
//        t1.start();
//    }
//
//    @Override
//    public void run() {
//                try{
//                    Thread.sleep(10000);
//                    this.setVisible(false);
//                }catch (Exception e)
//                {
//                    e.printStackTrace();
//                }
//
//    }
//}


package Electricity;

import java.awt.*;
import java.awt.event.*;
import javax.swing.*;

public class Splash {
    public static void main(String[] args){
        fframe f1 = new fframe();
        f1.setVisible(true);
        int i;
        int x=1;
        for(i=2; i<=600; i+=4, x+=1){
            f1.setLocation(800 - ((i+x)/2), 500 - (i/2));
            f1.setSize(i+x,i);
            try{
                Thread.sleep(10);
            }catch(Exception e){}
        }

    }
}
class fframe extends JFrame implements Runnable{
    Thread t1;
    fframe(){
        super("Electricity Billing System");
        setLayout(new FlowLayout());
        ImageIcon c1 = new ImageIcon(ClassLoader.getSystemResource("icon/desk.jpg"));
        Image i1 = c1.getImage().getScaledInstance(730, 550,Image.SCALE_DEFAULT);
        ImageIcon i2 = new ImageIcon(i1);


        JLabel l1 = new JLabel(i2);
        add(l1);
        t1 = new Thread(this);
        t1.start();
    }
    public void run(){
        try{
            Thread.sleep(7000);
            this.setVisible(false);

            Login l = new Login();
            l.setVisible(true);
        }catch(Exception e){
            e.printStackTrace();
        }
    }
}